...
class Enigma():
	...
	def set_ringkerben(self, rk=[]):
		"""set the Ringkerben for all Walzen, indicating on which letter the
	...
	def set_base(self, base):
		"""set the base for all Walzen, that is the initial position of the 
		Walzen. 
			...
			if lex_index >= 0 and lex_index <= 25:
				...
				for stecker in sb:
					if char in stecker:
						...
						if vverbose: print("		Steckerbrett:", char, "-->", new_char)
						break
				...
				for walze in walzen_curr:
					...
					if vverbose: print("			", alphabet)
					if vverbose: print("			", walze, "-->", new_letter)

				# reflect the current character on the selected Umkehrwalze
				new_letter = ukw[lex_index]
				lex_index = ord(new_letter) - 65
				if vverbose: print("				", alphabet)
				if vverbose: print("				", ukw, "-->", new_letter)
				...
				for walze in walzen_curr[-1::-1]:
					...
					lex_index = ord(new_letter) - 65
					if vverbose: print("			", walze)
					if vverbose: print("			", alphabet, "-->", new_letter)
				...
				for stecker in sb:
					if char in stecker:
						...
						if vverbose: print("		Steckerbrett:", new_letter, "-->", char)
						break
